...
heights = seq.int(1:30)
bufs = c(0,20,50,100,200)
min_gap_size = c(1, seq(10,200, by = 10))
  ...
  for (c in 1:length(niveaux_model[[3]])){
    for (d in 1:length(niveaux_model[[4]])){
      for (e in 1:length(niveaux_model[[5]])){
        ...
        if (var(data_toUse$proportion) == 0) {  # Check if all values in 'proportion' are the same
          model_list[[a]][["proportion"]][[c]][[d]][[e]] = NA  # Consider assigning NA or another default value
        } else {
          model_list[[a]][["proportion"]][[c]][[d]][[e]] = train(respons ~ proportion, data = data_toUse, method = 'lm', trControl = trainControl(method = "LOOCV"), preProcess = NULL)
        }
