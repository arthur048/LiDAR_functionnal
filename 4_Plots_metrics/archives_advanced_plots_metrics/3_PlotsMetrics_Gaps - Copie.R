...
#' Calcule l'aire sous la courbe de proportion vs hauteur entre deux hauteurs
...
#' 
#' @param data Données d'un plot filtré
#' @param min_height Hauteur minimale à considérer (défaut: minimum des données)
#' @param max_height Hauteur maximale à considérer (défaut: 60)
...
calculate_auc <- function(data, min_height = NULL, max_height = NULL, normalize = TRUE) {
  # Ordonner les données par hauteur
  data <- data %>% arrange(height_aboveground)
  
  # Définir height si non spécifié
  if (is.null(min_height)) {
    min_height <- min(data$height_aboveground)
    max_height <- max(data$height_aboveground)
  }
...
fit_logistic_model <- function(data) {
  ...
  tryCatch({
    ...
    p <- length(coef(model))  # Nombre de paramètres (4 dans votre cas)
    R2_adj <- 1 - (1 - R2) * ((n - 1) / (n - p - 1))
    
    AIC <- AIC(model)
    
    return(list(a = a, b = b, h0 = h0, k = k, R2 = R2, R2_adj = R2_adj, AIC = AIC, 
                convergence = TRUE))
  }, error = function(e) {
    # En cas d'échec de convergence, retourner NA
    return(list(a = NA, b = NA, h0 = NA, k = NA, R2 = NA, R2_adj = NA, AIC = NA, 
                convergence = FALSE))
  })
}
...
#' @param prop_at_5m Proportion de trouées à 5m
...
calculate_combined_metrics <- function(h10, h25, h50, h75, h90, prop_at_5m, Smax, hSmax, auc) {
  # Vérifier que h50 n'est pas NA ou 0 (pour éviter division par zéro)
  if (is.na(h50) || h50 == 0) {
    return(list(
      IDV = NA, ROS = NA, IAS = NA, ICT = NA, IPN = NA,
  ...
  # Ratio d'Occupation Stratifiée: élevé pour forêts à WD élevée (h50 élevé, prop_at_5m très faible)
  ROS <- h50 / (prop_at_5m + 0.005)
  ...
  return(list(
    IDV = IDV, ROS = ROS, IAS = IAS, ICT = ICT, IPN = IPN,
...
# Analyses préliminaires des points clés dans la distribution des hauteurs selon le WD

# Calculer la corrélation entre WD et proportion pour chaque hauteur
correlation_by_height <- combined_data %>%
  group_by(height_aboveground) %>%
  summarize(correlation = cor(WD_mean, proportion, method = "spearman"),
            correlation_abs = abs(correlation),
            p_value = cor.test(WD_mean, proportion, method = "spearman")$p.value,
            .groups = "drop") %>%
  arrange(desc(correlation_abs))
...
for (plot_i in plot_names) {
  ...
  combined_metrics <- calculate_combined_metrics(
    h10, h25, h50, h75, h90, prop_5m, Smax, hSmax, auc_val
  )
...
plot_metrics_all <- plot_metrics_basic %>%
  left_join(plot_metrics_advanced) %>%
  left_join(plot_metrics_optimized)
